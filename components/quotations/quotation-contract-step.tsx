...
export function QuotationContractStep({quotation} : {quotation: Quotation}) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const endpoint =  `${process.env.NEXT_PUBLIC_API_URL}/api/contracts/upload-signed/${quotation.id}`

            const response = await fetchWithTokenWithoutContentType(endpoint, {
                method: 'POST',
                body: formData,
            });
            ...
            toast({
                title: 'Succès',
                description:'Le contrat signé a été uploadé avec succès'
            });
    ...
    const handleDownloadContract = async () => {
        try {
            const response = await fetchWithToken(`${process.env.NEXT_PUBLIC_API_URL}/api/contracts/download/${quotation.id}`);
    ...
    if (quotation.contract && quotation.contract.signed) {
        return (
